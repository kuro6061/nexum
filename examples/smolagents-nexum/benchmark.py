"""
benchmark.py — Before/After comparison: plain smolagents vs smolagents + Nexum
...
def fmt_result(r: dict) -> str:
    ...
    if r["error"]:
        lines.append(f"  Status     : CRASHED — {r['error'][:60]}")
...
def main():
    ...
    print("PLAIN smolagents — crashes after step 1:")
    plain2 = run_plain(QUERY, crash_after=1, label="plain-crash")
    print(fmt_result(plain2))

    print("\nNexum — crashes after step 1 (but checkpoint saved):")
    ...
    print("PLAIN smolagents — must restart from scratch:")
    t0 = time.time()
    ...
    print("\nNexum — resumes from checkpoint (step 1 already cached):")
    t0 = time.time()
