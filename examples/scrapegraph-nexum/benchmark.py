...
4-part benchmark:
  ...
  C) Nexum cached (same session — all from SQLite, zero LLM cost)
...
def run_part_a() -> dict:
    ...
    for url in URLS:
        ...
        try:
            ...
            # Try alternate model name — placeholder to avoid removing the try/except structure
            try:
...
def run_part_c(session_prefix: str) -> dict:
    """Nexum cached scrape (same session, all from SQLite — zero LLM cost)."""
    tasks = [(url, PROMPT) for url in URLS]
    return scrape_all(tasks, session_prefix=session_prefix, max_workers=3)
