...
Directly addresses:
  - RAGAS #2473: "838 evaluations take 10 min despite 5k RPM" ↁENexum parallel
  - RAGAS #2136: "Support Batch API for lower cost" ↁENexum cache = $0 re-eval
  - RAGAS #2105: "Generate QA samples in parallel for big batches" ↁENexum EFFECT

4-part benchmark:
  A) Plain sequential: evaluate samples one-by-one (no Nexum)
  B) Nexum fresh:      same samples as parallel EFFECTs (first run)
  C) Nexum cached:     same session re-run  Eall from SQLite, zero LLM cost
  D) Crash sim:        crash after K completions, resume ↁEonly K' LLM calls
...
def run_part_a(samples: list[dict]) -> tuple[list[dict], int]:
    """Plain sequential evaluation  Eno parallelism, no crash recovery."""
    results = []
    for s in samples:
...
def main():
    ...
    for r in results_b:
        if r and r.get("faithfulness", 1.0) < 0.5:
            print(f"  ⚠ {r['sample_id']}: faithfulness={r['faithfulness']:.2f}   E{r.get('faithfulness_reason', '')[:80]}")
