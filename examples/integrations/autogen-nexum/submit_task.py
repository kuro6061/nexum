"""
submit_task.py  ESubmit an AutoGen tool call as a Nexum workflow execution.
...
def submit_tool_call(
    ...
    immediately  Ethis is the crash-recovery mechanism.
    ...
    if exec_id:
        ...
        if status["status"] == "COMPLETED":
            ...
            print(f"[Nexum] {key} already COMPLETED (cached) ✁E)
            client.close()
            return result
    ...
    # Adaptive polling: tight (50ms) for first 2s ↁErelaxed (200ms) after
    ...
    while time.time() < deadline:
        ...
        if st == "COMPLETED":
            result = status["completedNodes"].get("tool_call", {}).get("result", "")
            print(f"[Nexum] {exec_id[:16]}... done ✁E)
            client.close()
            return result
