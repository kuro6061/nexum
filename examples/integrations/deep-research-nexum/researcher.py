...
Workflow:
  1. generate_queries(topic)     -- 1 LLM call ↁEN sub-queries
  2. search_and_learn ÁEN        -- N parallel Nexum EFFECTs (crash-safe)
  3. merge_learnings(results)    -- local, free
  4. write_report(topic, learnings) -- 1 LLM call ↁEMarkdown report

Crash recovery: completed EFFECTs are cached in SQLite.
Re-running the same (topic, session) skips completed queries  Ezero LLM cost.
...
def research(
    ...
    1. Generate N sub-queries (1 LLM call)  Esaved to session for deterministic re-runs
    ...
    # Step 1: Generate sub-queries  Eload from session if already generated
    session = _load_session(session_prefix)
