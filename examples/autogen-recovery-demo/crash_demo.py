"""
crash_demo.py — Demonstrates crash recovery for AutoGen #7043.
...
This script:
  ...
  4. Shows that Nexum resumes from 'summarize' — not from the beginning
...
from workflow import research_report_workflow
...
def main():
    ...
    banner("Step 1: Starting pipeline execution", "═")
    ...
    print(f"  Pipeline:     research → validate → summarize → report")

    # ── Step 2: Start worker #1 — will crash after specified node ──
    banner(f"Step 2: Worker #1 starts (will crash after '{args.crash_after}')", "═")
    ...
    if worker1.returncode == 0:
        print("  [WARNING] Worker exited cleanly — crash simulation may not have triggered.")
        print("  Continuing anyway...")
    else:
        print(f"  ✓ Worker #1 crashed as expected (simulating SIGKILL / OOM)")

    # ── Step 3: Verify partial progress ───────────────────────────
    banner("Step 3: Checking partial progress in Nexum DB", "═")
    ...
    print(f"    → Resume → 'execution is complete' (stuck, wrong)")
    print(f"\n  With Nexum:")
    print(f"    → Resume → picks up from next pending node (correct)")

    # ── Step 4: Fresh worker resumes ──────────────────────────────
    banner("Step 4: Worker #2 starts (no crash — fresh, naive worker)", "═")
    ...
    print(f"  Resume → done in: {t_done - t_resume:.1f}s")
    print()
    ...
    for step in STEPS:
        if step in final_nodes:
            ...
            resumed = " ← resumed here" if step == STEPS[crash_idx + 1] else ""
            print(f"    {'✓':2} {step:12}  ({worker_label}){resumed}")
        else:
            print(f"    {'✗':2} {step:12}  (not completed)")

    print()
    skipped = [s for s in completed if s in final_nodes]
    resumed_from = STEPS[crash_idx + 1] if crash_idx + 1 < len(STEPS) else None
    if resumed_from:
        print(f"  ✓ Worker #2 started from '{resumed_from}' — not from the beginning")
        print(f"  ✓ Nodes re-executed by Worker #2: {[s for s in STEPS if s not in completed and s in final_nodes]}")
